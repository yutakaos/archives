...
# 2. linear VAR model with system noise
# 3. Ricker model with system noise
test = function (tl = 100, eLib = 0.1, eTar = eLib, N = 1000, model="")
{
    if (!model%in%c("Logistic","VAR","Ricker")) stop("Invalid model is specified.")
    if (model=="Logistic") { make_data <<- make_data_2sp_logistic; Bxy <- 0.1 }
    if (model=="VAR")      { make_data <<- make_data_2sp_var     ; Bxy <- 0.5 }
    if (model=="Ricker")   { make_data <<- make_data_2sp_ricker  ; Bxy <- 0.1 }
...
experiment1 <- function(e0, e1, model)
{
    rbind(
...
df_1a <- experiment1(e0=0.05, e1=0.5, model="Logistic")
df_1b <- experiment1(e0=1.0 , e1=2.0, model="VAR")
df_1c <- experiment1(e0=0.05, e1=0.5, model="Ricker")
df_2a <- experiment2(e0=0.05, model="Logistic")
df_2b <- experiment2(e0=1.0 , model="VAR")
df_2c <- experiment2(e0=0.05, model="Ricker")
...
gg_fun <- function(df1, df2) {
    ...
    out <- gp_1/gp_2 + plot_layout(height=c(2,1))
    out
}
print(gp_1a <- gg_fun(df_1a, df_2a))
print(gp_1b <- gg_fun(df_1b, df_2b))
print(gp_1c <- gg_fun(df_1c, df_2c))
ggsave("figs/Fig_2a.png", plot=gp_1a, width=4, height=6.4)
ggsave("figs/Fig_2b.png", plot=gp_1b, width=4, height=6.4)
ggsave("figs/Fig_2c.png", plot=gp_1c, width=4, height=6.4)
